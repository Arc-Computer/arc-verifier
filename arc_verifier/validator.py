...
class TEEValidator:
    ...
    def _generate_quote(self, image_tag: str) -> TEEQuote:
        ...
        signature = hashlib.sha256(
            f"quote-sig-{base_hash}-{datetime.now().isoformat()}".encode()
        ).hexdigest()

        # Generate nonce for freshness
        nonce = hashlib.sha256(f"nonce-{base_hash}-{time.time()}".encode()).hexdigest()[
            :16
        ]
    ...
    def _calculate_trust_level(self, is_valid: bool, image_tag: str) -> str:
        ...
        if any(
            pattern in image_tag.lower() for pattern in ["shade", "pivortex", "near"]
        ):
            return "HIGH"

        # Medium trust for standard images
        if any(
            pattern in image_tag.lower() for pattern in ["nginx", "ubuntu", "alpine"]
        ):
            return "MEDIUM"
