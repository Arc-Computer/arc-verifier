...
class DockerScanner:
    ...
    def __init__(self, force_mock=False):
        ...
        if force_mock:
            ...
            try:
                ...
                self.console.print(f"[red]❌ Docker daemon connection failed: {e}[/red]")
                self.console.print("[red]Arc-Verifier requires Docker to be running.[/red]")
                self.console.print("[yellow]Please ensure Docker daemon is started and accessible.[/yellow]")
                raise RuntimeError(f"Docker is required for Arc-Verifier operation. Error: {e}")
    ...
    def scan(self, image_tag: str) -> Dict[str, Any]:
        ...
        if not self.docker_available:
            ...
            raise RuntimeError("Docker daemon is not available. Cannot proceed with scanning.")
    ...
    def _run_trivy_scan(self, image_tag: str) -> List[Vulnerability]:
        ...
        try:
            ...
            self.console.print("[yellow]Install Trivy: https://aquasecurity.github.io/trivy/latest/getting-started/installation/[/yellow]")
            raise RuntimeError("Trivy vulnerability scanner is required but not installed")
            ...
            for i, layer_digest in enumerate(layer_digests):
                ...
                if i < len(history):
                    ...
                    if created_by.startswith('/bin/sh -c'):
                        command = created_by[11:60] + ('...' if len(created_by) > 60 else '')
                    else:
                        command = created_by[:60] + ('...' if len(created_by) > 60 else '')
    ...
    def _detect_shade_agent(self, image: docker.models.images.Image) -> bool:
        ...
        try:
            ...
            agentic_env_patterns = [
                # NEAR/Shade specific
                'SHADE_', 'ARC_AGENT', 'NEAR_', 'TEE_',
                # General agentic patterns
                'AGENT_', 'BOT_', 'TRADING_', 'DEFI_',
                # Protocol specific
                'ETHEREUM_', 'SOLANA_', 'POLYGON_', 'AVALANCHE_',
                # AI/ML agent patterns
                'MODEL_', 'INFERENCE_', 'OPENAI_', 'ANTHROPIC_',
                # Trading/finance specific
                'PRIVATE_KEY', 'WALLET_', 'STRATEGY_', 'RISK_'
            ]
            ...
            agentic_label_patterns = [
                # Core patterns
                'shade', 'arc', 'near', 'agent', 'bot',
                # Trading/DeFi
                'trading', 'defi', 'finance', 'swap', 'liquidity',
                # Protocols
                'ethereum', 'solana', 'polygon', 'arbitrum',
                # AI/ML
                'ai', 'ml', 'model', 'inference', 'gpt',
                # Agentic frameworks
            ...
            agentic_file_patterns = [
                # NEAR/Shade specific
                '@neardefi/shade-agent-js', 'shade-agent', 'near-api-js', 'chainsig.js',
                # Web3 libraries
                'web3.js', 'ethers.js', 'viem', '@solana/web3.js',
                # Trading/DeFi libraries
                'ccxt', '1inch', 'uniswap', 'pancakeswap', 'sushiswap',
                # AI/ML libraries
                'openai', 'anthropic', 'langchain', 'autogen', 'crewai',
                'transformers', 'torch', 'tensorflow',
                # Agentic frameworks
                'agent-framework', 'multi-agent', 'swarm',
                # Common patterns
                '/app/pages', '/app/agents', '/app/strategies',
                'yarn dev', 'npm run dev', 'npm run agent',
                # Config files
                'agent.config', 'strategy.config', 'trading.config',
                '.env.agent', '.env.trading'
            ]
            ...
            for tag in image_tags:
                tag_lower = tag.lower()
                tag_patterns = [
                    'shade', 'agent', 'bot', 'trading', 'defi',
                    'finance', 'swap', 'arbitrage', 'market-maker',
            ...
            self.console.print(f"[yellow]Agentic protocol detection failed: {e}[/yellow]")
            return False
    ...
    def _get_mock_scan_result(self, image_tag: str) -> Dict[str, Any]:
        ...
        agentic_keywords = [
            ...
            'shade', 'near', 'pivortex',
            # General agentic
            'agent', 'bot', 'trading', 'defi', 'finance',
            # AI/ML
            'ai', 'ml', 'gpt', 'claude', 'llm',
            # DeFi/Trading
            'swap', 'arbitrage', 'market-maker', 'liquidity'
        ]
        shade_detected = any(keyword in image_tag.lower() for keyword in agentic_keywords)
        ...
        mock_result = ScanResult(
            ...
            layers=[
                LayerInfo(index=0, digest="sha256:abc123", size=5242880, command="FROM node:18-alpine"),
                LayerInfo(index=1, digest="sha256:def456", size=1048576, command="COPY package.json ."),
                LayerInfo(index=2, digest="sha256:ghi789", size=52428800, command="RUN npm install"),
            ],
            shade_agent_detected=shade_detected,
